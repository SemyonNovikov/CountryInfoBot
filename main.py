...
translate = YandexTranslate(configs.yandex_translate_key)

# ключ vk api
token_vk = configs.token_vk

# ключ для конвертации через api cloudconvert
api = cloudconvert.Api(configs.cloudconvert_token)
...
def translator(text,
               direction):  # Функция переводчик , аргументы : text - что перевести , direction - направление перевода
    try:
    ...
    if hello == 'привет':
        ...
        try:
            ...
            if find_en == 'currencies' or find_en == 'languages':
                ...
                path_to_jpg = 'Photos/' + country + '.jpg'
                # путь для SVG картинки
                path_to_svg = 'Photos/' + country + '.svg'
